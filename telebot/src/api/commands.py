...
from src.domain.telegram_chatter import TelegramChatter
from src.db.models import MessageTypeEnum, RatingEnum

from src.api.keyboard import get_delete_confirmation_keyboard, get_rating_keyboard, get_main_menu_keyboard, generate_thread_keyboard
...
async def callback_query_handler(update: Update, context):
    ...
    async with rethinkdb_connection() as connection:
        ...
        if data == "show_chats":
            ...
            print("Вход в блок обработки лайков")
            parts = data.split("_")
            message_id = parts[1]
            rating_str = parts[2]
...
async def user_message(update: Update, context):
    ...
    async with rethinkdb_connection() as connection:
        ...
        try:
            print("Начало запроса к основной модели")
            llm = ChatOpenAI(
            ...
            print(f"респонс основной модели{response}")
